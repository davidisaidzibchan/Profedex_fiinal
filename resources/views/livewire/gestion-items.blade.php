<div class="flex flex-col justify-center items-center w-full">
    <div class=" absolute top-14 h-14 bg-purple-600 text-white w-full py-3 px-6">
        <h1 class="text-xl font-bold">Gestión de items</h1>
    </div>
    <div class="bg-gradient-to-b mt-14 py-5 from-blue-400 to-purple-600 w-full p-3 shadow-lg"
            ...
            <div class="fixed z-10 inset-0 overflow-y-auto">
                <div class="flex items-center justify-center min-h-screen">
                    ...
                    <div class="bg-white mt-14 rounded-lg shadow-lg overflow-hidden max-w-lg w-full z-20">
                        <div class="p-3">
                            <h2 class="text-2xl font-bold mb-4">{{ $idItem ? 'Editar item' : 'Crear item' }}</h2>
                            <form>
                                <div class="mb-4">
                                    <label for="prefijo" class="block text-gray-700">Prefijo:</label>
                                    <select id="prefijo" wire:model="id_profesor"
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="flex flex-col gap-1">
                                        <div class="w-32 h-32 rounded-xl overflow-hidden">
                                            @if ($ruta instanceof \Illuminate\Http\UploadedFile)
                                                <img src="{{ $ruta->temporaryUrl() }}" alt="Imagen"
                                                    class="w-full h-full object-cover mb-2">
                                            @elseif ($ruta)
                                                <img src="{{ asset('storage/' . $ruta) }}" alt="Imagen"
                                                    class="w-full h-full object-cover mb-2">
                                            @else
                                                <img src="{{ asset('img/image-placeholder.svg') }}"
                                                    alt="Imagen por defecto" class="w-full h-full object-cover mb-2">
                                            @endif
                                        </div>
                                        <label for="imagen"
                                            class="bg-amber-600 text-center w-32 px-3 py-2 text-white rounded-xl">Subir
                                            archivo</label>
                                        <input type="file" id="imagen" wire:model="ruta" class="hidden">
                                        @error('ruta')
                                            <span class="text-red-500">{{ $message }}</span>
                                        @enderror
                                    </div>
                ...
                <div
                    ...
                    <div class="modal-content py-4 text-left px-6">
                        ...
                        <div class="mt-4 flex justify-end">
                            ...
                            <button wire:click="closeConfirmDelete()" class="px-4 py-2 bg-gray-400 text-white rounded">
                                Cancelar
                            </button>
                        </div>
                    </div>
        ...
        <div class="w-full overflow-auto text-xs">
            @foreach ($items->groupBy('id_profesor') as $profesorId => $profesorItems)
                @php
                    $profesor = App\Models\Profesor::find($profesorId);
                @endphp
                <div class="mb-4">
                    <h2 class="text-xl underline font-semibold mb-10">{{ $profesor->nombre }}</h2>
                    <div class="flex flex-wrap gap-4 justify-center items-center">
                        @foreach ($profesorItems as $item)
                            <div
                                class="flex md:w-44 w-28 flex-col items-center justify-center bg-white rounded-xl overflow-hidden">
                                <div class="w-full bg-slate-400">
                                    <img class="h-full w-full object-cover"
                                        src="{{ asset('storage/' . $item->ruta) }}" alt="item" />
                                </div>
                                <div class="flex md:flex-row flex-col w-full">
                                    <button wire:click="edit('{{ $item->id }}')"
                                        class="flex md:w-1/2 w-full items-center justify-center gap-1 p-2 cursor-pointer md:border-r bg-blue-600 text-white hover:bg-blue-500 active:bg-blue-700">
                                    ...
                                    <button wire:click="confirmDelete('{{ $item->id }}')"
                                        class="flex md:w-1/2 w-full items-center justify-center gap-1 p-2 cursor-pointer bg-red-600 text-white hover:bg-red-500 active:bg-red-700">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                            fill="currentColor" class="w-5 h-5">
